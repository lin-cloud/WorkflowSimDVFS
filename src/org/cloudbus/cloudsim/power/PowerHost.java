...
public class PowerHost extends HostDynamicWorkload {
	...
	public double getEnergyLinearInterpolation(double fromUtilization, double toUtilization, double time)
	{
		...
		if(ExportCsv.doExport() && (ExportCsv.getIndex() == getId()))
		{
			ExportCsv.setTempUtilization(toUtilization);
			ExportCsv.setTempPower(toPower);
			ExportCsv.setTempMultiplier(getPeList().get(0).getIndexFreq());
		}
